...
extern void Testcase(double *A, double *B, double *Sig, char *name,
                     int *p, int *q, int *r, int icase, rand_rng *rng);
...
static int name_to_icase(const char *s) {
    static const char *names[] = {
        "tinyAR","tinyMA","tinyARMA","smallAR","smallMA","smallARMA1",
...
static int get_options(int argc, char **argv,
                       char *out_testcase, size_t out_tsize, int *out_n) {
    if (argc < 2) {
    ...
    while ((opt = getopt(argc, argv, "hn:Pe:p")) != -1) {
        switch (opt) {
            ...
            case 'n': {
                int tmp = atoi(optarg);
                assert(tmp > 0 && "Number of terms must be positive");
                n = tmp;
                break;
            }
            case 'P':
...
int main(int argc, char **argv) {
    ...
    /* Decide icase and/or dimensions */
    int p = 0, q = 0, r = 0;
    int icase = -1;
    ...
    assert(r != 0 && "Testcase returned zero dimension (r=0)");
    assert(icase >= 0 && "Testcase index (icase) invalid");
    ...
    /* Fill mu with a simple pattern (optional — not used yet) */
    for (int i = 0; i < r; ++i)
        mu[i] = (double)(i + 1);
