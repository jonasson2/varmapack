...
static void check_acvf_vector_var1_diagonal(void) {
  ...
  bool ok = varmapack_acvf(A, 0, Sig, 1, 0, 2, Gamma, 3);
  checkVarmapackSuccess(ok);
...
static void check_acvf_vector_ma1(void) {
  ...
  bool ok = varmapack_acvf(0, B, Sig, 0, 1, 2, Gamma, 2);
  checkVarmapackSuccess(ok);
...
static void check_acvf_maxlag_equals_p(void) {
  ...
  double Sig[] = {2};
  double Gamma[3];
  double expected[] = {2.5462962962962967, 1.1574074074074074, 0.3240740740740741};
  bool ok = varmapack_acvf(A, 0, Sig, 2, 0, 1, Gamma, 2);
  checkVarmapackSuccess(ok);
  checkArrayTol(Gamma, expected, 3, 1e-12);
}

static void check_acvf_maxlag_below_p(void) {
  double A[] = {0.5, -0.1};
  double Sig[] = {2};
  double Gamma[1];
  double expected[] = {2.5462962962962967};
  bool ok = varmapack_acvf(A, 0, Sig, 2, 0, 1, Gamma, 0);
  checkVarmapackSuccess(ok);
  checkArrayTol(Gamma, expected, 1, 1e-12);
}
...
static void check_acvf_nonstationary_failure(void) {
  ...
  double Sig[] = {1};
  double Gamma[1];
  bool ok = varmapack_acvf(A, 0, Sig, 1, 0, 1, Gamma, 1);
...
static void check_acvf_invalid_input(void) {
  ...
  double Gamma[2];
  bool ok;
  ok = varmapack_acvf(0, 0, Sig, 1, 0, 1, Gamma, 1);
  checkVarmapackFailure(ok);
  ok = varmapack_acvf(A, 0, Sig, 1, 1, 1, Gamma, 1);
  checkVarmapackFailure(ok);
  ok = varmapack_acvf(A, B, 0, 1, 1, 1, Gamma, 1);
  checkVarmapackFailure(ok);
  ok = varmapack_acvf(A, B, Sig, 1, 1, 1, 0, 1);
  checkVarmapackFailure(ok);
  ok = varmapack_acvf(A, B, Sig, -1, 1, 1, Gamma, 1);
  checkVarmapackFailure(ok);
  ok = varmapack_acvf(A, B, Sig, 1, -1, 1, Gamma, 1);
  checkVarmapackFailure(ok);
  ok = varmapack_acvf(A, B, Sig, 1, 1, 0, Gamma, 1);
  checkVarmapackFailure(ok);
  ok = varmapack_acvf(A, B, Sig, 1, 1, 1, Gamma, -1);
  checkVarmapackFailure(ok);
}
