...
static void check_scalar_maxlag(void) {
  ...
  bool ok;
  ok = varmapack_autocov("N", "ML", 1, 3, X, 2, C);
  checkVarmapackSuccess(ok);
  checkArrayTol(C, ml, 3, 1e-12);
  ok = varmapack_autocov("N", "C", 1, 3, X, 2, C);
  checkVarmapackSuccess(ok);
...
static void check_constant_series(void) {
  ...
  double C[2*2*4];
  bool ok;
  ok = varmapack_autocov("N", "ML", 2, 4, X, 3, C);
  checkVarmapackSuccess(ok);
  checkArrayZero(C, 16);
  ok = varmapack_autocov("N", "C", 2, 4, X, 3, C);
  checkVarmapackSuccess(ok);
  checkArrayZero(C, 16);
}
...
static void check_maxlag_zero(void) {
  ...
  bool ok;
  ok = varmapack_autocov("N", "ML", 2, 3, X, 0, C);
  checkVarmapackSuccess(ok);
  checkArrayTol(C, expected, 4, 1e-12);
  ok = varmapack_autocov("N", "C", 2, 3, X, 0, C);
  checkVarmapackSuccess(ok);
  checkArrayTol(C, expected, 4, 1e-12);
}
...
static void check_single_observation(void) {
  ...
  double C[4];
  bool ok;
  ok = varmapack_autocov("N", "ML", 2, 1, X, 0, C);
  checkVarmapackSuccess(ok);
  checkArrayZero(C, 4);
  ok = varmapack_autocov("N", "C", 2, 1, X, 0, C);
  checkVarmapackSuccess(ok);
  checkArrayZero(C, 4);
}
