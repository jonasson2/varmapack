...
//   ncase, p[], q[], r[], n[], and pointer arrays A[], B[], Sig[], X[] and more
...
#include "matlabcompare.inc"
  ...
  for (int k = 0; k < ncases; k++) {
    ...
    allocate(X0sim, rk*nk*2);
    allocate(Esim,  rk*nk);
    allocate(EEsim,  rk*nk*2);
    allocate(E0sim,  rk*nk*2);
    allocate(x0, rk*h);
    startmat(x0, rk, h);
    ...
    varmapack_sim(Ak, Bk, Sigk, mu, pk, qk, rk, nk, M, 0, 0, rng, Xsim, Esim, &sim_ok);
    varmapack_sim(Ak, Bk, Sigk, mu, pk, qk, rk, nk, M, 0, 0, rng, XXsim, EEsim, &sim_ok);
    varmapack_sim(Ak, Bk, Sigk, mu, pk, qk, rk, nk, M, x0, h, rng, X0sim, E0sim, &sim_ok);
    // printM("Xsim", Xsim, rk, nk*M);
    // printM("XXsim", XXsim, rk, nk*M);
    printM("X0sim", X0sim, rk, nk*M);
    xCheck(sim_ok == 1);
    // Compare E and X
    ...
    xCheck(relabsdiff(EEsim, EE[k], rk*nk*M) < tol*condR[k]);
    xCheck(relabsdiff(XXsim, XX[k], rk*nk*M) < tol*condR[k]);
    xCheck(relabsdiff(E0sim, E0[k], rk*nk*M) < tol*condR[k]);
    xCheck(relabsdiff(X0sim, X0[k], rk*nk*M) < tol*condR[k]);
