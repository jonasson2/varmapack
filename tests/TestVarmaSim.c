// TestVarmaSim  Test correctness of VarmaSim.c
//
//   DESCRIPTION
//     TestVarmaSim carries out several different tests of the function VarmaSim.
//     All the tests are carried out for the 8 first testcases from Testcase.c.
...
//     VarmaSim.
//     TestVarmaSim prints theoretical covariance, data covariance of simulated
...
int TestVarmaSim(char *code) { // code for example = "PF" for print, full,
  ...
  ifail = 0;
  if (PRINT) { printf("\n  Test VarmaSim"); fflush(0); }
  while (true) {
    dmax = 0;
    Testcase(0, 0, 0, testname, &p, &q, &r, icase, rng); // find dimensions
    prints(testname);
    ...
    Testcase(A, B, Sig, testname, &p, &q, &r, icase, rng);
    h = max(p,q);
    //
    ...
    allocate(mu3, r*n);
    allocate(mud, r);
    VarmaSim(A, B, Sig, mu, p, q, r, n, M, 0, rng, X, 0, &OK); xAssert(OK);
    mean3(X, r, n, M, 3, mu3);
    ...
    VarmaSim(A, B, Sig, 0, p, q, r, n, 1, 0, rng, X, 0, &OK); xAssert(OK);
    allocate(XX, (n-1)*r*2);
    ...
    VarmaSim(A, B, Sig, 0, p, q, r, n, M, 0, rng, X, 0, &OK); xAssert(OK);
    ...
    allocate(X, r*n*M);
    allocate(eps, r*n*M);
    VarmaSim(A, B, Sig, 0, p, q, r, n, M, 0, rng, X, eps, &OK); xAssert(OK);
    allocate(CC, r*h * r*h);
    allocate(SS, r*h * r*h);
    allocate(X1, M*r*h*2);
    allocate(X2, M*r*h*2);
    ...
    VarmaSim(A, B, Sig, mu, p, q, r, h+2, M, X0, rng, X, 0, &OK); xAssert(OK);
