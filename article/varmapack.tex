...
+\sum_{i=0}^s C_i z_{t-i},
$$
where the covariates $z_t$ are known and
$$
...
x_0,\ldots,x_{h-1}
$$
are available and that $h$ is the first prediction time. Assume also that
$h>\max(p,s)$, so that at least one residual equation is available. The startup shocks
$$
E_0=(\varepsilon_0',\ldots,\varepsilon_{h-1}')'
$$
are then estimated conditionally on the observed startup segment.

The first usable equation is at time
$$
t_0=\max(p,s),
$$
and for $t=t_0,\ldots,h-1$ define
$$
r_t
=
x_t
...
where
$$
r=(r_{t_0}',\ldots,r_{h-1}')'
$$
and $H$ is the block lower-triangular matrix with block entries
$$
H_{t,j}
=
\begin{cases}
I_r, & j=t,\\
B_{t-j}, & 1\le t-j\le q,\\
0, & \text{otherwise},
\end{cases}
$$
for $t=t_0,\ldots,h-1$ and $j=0,\ldots,h-1$.

The stacked startup shocks satisfy
$$
E_0 \sim N(0,\Sigma'),
$$
where
$$
\Sigma' = I_h \otimes \Sigma.
$$
...
W = H \Sigma' H',
$$
which is the block Toeplitz covariance matrix of the MA($q$) process
$$
r_t
=
\varepsilon_t
+\sum_{i=1}^q B_i\varepsilon_{t-i}.
$$
Using the lower-triangular convention of Section~2, $W$ is a block Toeplitz
matrix whose $(i,j)$ lower block is $W_{i-j}$. The lag-$k$ covariance blocks are
$$
W_k
=
\operatorname{cov}(r_t,r_{t-k})
=
\sum_{i=0}^{q-k} B_{i+k} \Sigma B_i',
\qquad
k=0,\ldots,q,
$$
where $B_0=I_r$, and $W_k=0$ for $k>q$.

Let $L_W$ denote the Cholesky factor of $W$, so that $W=L_WL_W'$.

The conditional mean and covariance of the startup shocks are therefore
$$
\hat E_0
=
\Sigma' H' W^{-1} r
...
The startup shocks are drawn from this distribution and simulation then proceeds recursively from time $t=h$.
