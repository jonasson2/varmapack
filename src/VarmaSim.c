...
void VarmaSim( //  Generate simulated AR, VAR, ARMA or VARMA time series
  ...
  rand_rng *rng,// in/out  random number generator
  ...
  int *ok)      // out  1 if the series is stationary or X0 is non-null, else 0
{
  ...
  //
  // ABOUT THE PROGRAM:
  //   The program is a translation of the Matlab program varma_sim described
  //   in references [1], [2] and [3] (the report [3] contains some details not
  //   found in the published papers).
  //
  // STARTING VALUES:
  //   When X0 is specified, it is used to start the simulation. Each generated
  //   series will start with X0, and subsequent terms are simulated using X0
  //   as input. An important feature is that  the simulated series have an
  //   accurate distribution from term one (they are spin-up free). Thus there
  //   is no need to throw a way an initial segent of the simulated series. If
  //   X0 is not used, the first h x(t) and eps(t) values (with h = max(p,q))
  //   are drawn from the correct joint distribution of these variables, and if
  //   X0 is used the first h eps(t) values are drawn from the correct
  //   conditional distribution of eps(1)...eps(h)|x(1)...x(h).
  //
  // MATRIX STORAGE SCHEME
  //   All matrices are stored in Fortran fashion (i.e. column major ordering),
  //   so that for example the elements of A1, A2... are stored in the order
  //   A1(1,1), A1(2,1),...,A1(r,r),..., Ap(r,r).
  //
  // STATIONARITY
  //   If X0 is not specified, the parameter matrices Ai and Bi must represent
  //   a stationary model, but if X0 is specified the series is allowed to be
  //   non-stationary.
  //
  // FAILURES
  //   If the series is nonstationary or Sig is not positive semidefinite then
  //   the function returns (quietly) with ok = 0 (false). In exceptional cases,
  //   (which should only be possible when X0 is specified and the series is
  //   nonstationary) needed matrices may be singular, and then the function
  //   exits by calling one of the function defined in xAssert.h, which are
  //   expected to print an error message and terminate the program.
  //
  // REFERENCES:
