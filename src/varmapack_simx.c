...
static varmapack_error drawStartupShocks(double A[], double B[], double C[], double Sig[],
                                         double z[], double X0[], double Eall[],
                                         int p, int q, int s, int r, int h, int t0,
                                         int firstShockTime, int firstActiveShock,
                                         int ldE, int M, randompack_rng *rng);
static bool forwardSimx(double Aflp[], double Bflp[], double C[], double Sig[],
                        double z[], double Eall[], int ldE, double X[], int p,
...
varmapack_error varmapack_simx(double A[], double B[], double C[], double Sig[],
                               double z[], int p, int q, int s, int r, int n,
                               int M, double X0[], int h, double X[], double E[],
                               randompack_rng *rng)
{
  ...
  flipmat(A, Aflp, r, p);
  flipmat(B, Bflp, r, q);
  for (int j=0; j<M; j++) copy(r*h, X0, 1, X + j*rn, 1);
  error = drawStartupShocks(A, B, C, Sig, z, X0, Eall, p, q, s, r, h, t0,
                            firstShockTime, firstActiveShock, ldE, M, rng);
  if (error) goto fail;
  if (!forwardSimx(Aflp, Bflp, C, Sig, z, Eall, ldE, X, p, q, s, r, n, M, h,
                   firstShockTime, rng)) {
    goto alloc_fail;
  }
  if (E != 0) {
...
static varmapack_error drawStartupShocks(double A[], double B[], double C[], double Sig[],
  ...
  if (q == 0) {
    ...
    startupResidual(A, C, z, X0, p, s, r, h, t0, residual);
    for (int j=0; j<M; j++) copy(rm, residual, 1, Eall + j*ldE + activeOffset, 1);
    FREE(residual);
    return VARMAPACK_OK;
  }
...
static bool forwardSimx(double Aflp[], double Bflp[], double C[], double Sig[],
  ...
  for (int t=h; t<n; t++) {
    ...
    for (int i=1; i<=s; i++) {
      for (int j=0; j<M; j++) {
        axpy(r, z[t-i+1], C + (i-1)*r, 1, X + j*rn + iX, 1);
      }
    }
  }
  return true;
