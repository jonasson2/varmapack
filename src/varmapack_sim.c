...
static varmapack_error startFromX0(double A[], double Sig[], double X0[], double mu[],
                                   ...
                                   int p, int q, int r, int h, int n, int M,
                                   randompack_rng *rng);
...
varmapack_error varmapack_sim(double A[], double B[], double Sig[], double mu[],
                              ...
                              int nX0, double X[], double E[], randompack_rng *rng)
{
...
static varmapack_error startFromX0(double A[], double Sig[], double X0[], double mu[],
                                   ...
                                   q, int r, int h, int n, int M, randompack_rng *rng) {
  ...
  subtractMean(x0bar, mu, nmu, r, h, 1, rh);
  for (int j=0; j<M; j++) {
  ...
  if (!randompack_mvn("T", e, R, rh, M, E, ldE, 0, rng)) {
    goto alloc_fail;
  }
