// src/varmapack_FindCG_gateway.c
#include <R.h>
...
// void varmapack_FindCG(double A[], double B[], double Sig[], int p, int q, int r,
//              double C[], double G[], double W[]);

SEXP varmapack_FindCG_gateway(SEXP A_, SEXP B_, SEXP Sig_) {
    ...
    if (TYPEOF(Adim) != INTSXP || LENGTH(Adim) != 3)
        Rf_error("varmapack_FindCG_gateway: A must be a 3D array with dims (r, r, p).");
    if (TYPEOF(Bdim) != INTSXP || LENGTH(Bdim) != 3)
        Rf_error("varmapack_FindCG_gateway: B must be a 3D array with dims (r, r, q).");
    if (TYPEOF(Sdim) != INTSXP || LENGTH(Sdim) != 2)
        Rf_error("varmapack_FindCG_gateway: Sig must be a 2D matrix with dims (r, r).");
    ...
    if (rA0 != rA1) Rf_error("varmapack_FindCG_gateway: A is not square in first two dims.");
    if (rB0 != rB1) Rf_error("varmapack_FindCG_gateway: B is not square in first two dims.");
    if (rS0 != rS1) Rf_error("varmapack_FindCG_gateway: Sig is not square.");
    int r = rA0;
    if (rB0 != r || rS0 != r)
        Rf_error("varmapack_FindCG_gateway: A, B, Sig must share the same 'r'.");
    ...
    // Call the core
    varmapack_FindCG(REAL(A_), REAL(B_), REAL(Sig_), p, q, r,
            REAL(C_), REAL(G_));
