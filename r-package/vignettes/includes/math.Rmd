...
$h=\max(p,q)$, and (b) by specifying $h \geq\max(p,q)$ initial values of the
series and drawing the first $h$ shocks from the conditional distribution of
$(\varepsilon|x)$.

For a stationary model, both (a) and (b) are possible. In case (a), the
simulated series has its exact unconditional distribution from the first term;
in case (b), it has its exact conditional distribution given the supplied
starting values. Thus, no burn-in segment needs to be discarded. In case (b),
the covariance of the startup states must be positive definite.
...
are present, the startup innovations are drawn from their theoretical
distribution conditional on any linear constraints implied by the supplied
states; in this case, $\Sigma$ must be positive definite. In the other VARMA
simulation cases, $\Sigma$ may be positive semidefinite and singular.

With VARMAX simulation, choose $h\geq\max(p,q,s-1)$ and supply the startup
states $x_t$, $t=0,\ldots,h-1$, together with the whole sequence of exogenous
values $z_t$, $t=0,\ldots,n-1$. When $h=0$, the startup segment is empty and no
states need be supplied. As for nonstationary VARMA with MA terms, $\Sigma$ must
be positive definite. The startup innovations are drawn from their theoretical
distribution conditional on any constraints imposed by the model and the
supplied states and exogenous sequence. Although $h=\max(p,q,s-1)$ is
sufficient, a larger $h$ should be used when a longer joint history of $x_t$ and
$z_t$ is available, since it provides more information for drawing the
innovations.
